...
***********************************************************************
*
*  Timing program for the REAL             Level 3 BLAS.
*
...
*  Data file for timing program for the REAL             Level 3 BLAS
...
*  SGEMM     T           ('T' or 't' means 'Time this routine')
*  SSYMM     T
*  SSYRK     T
*  SSYR2K    T
*  STRMM     T
...
*     .. External Subroutines ..
      EXTERNAL           PRNTAB, STIM01, STIM02, STIM03, STIM04, STIM05,
     $                   STIM06
*     .. Intrinsic Functions ..
      INTRINSIC          REAL
*     .. Data statements ..
...
*
   ...
   90       CONTINUE
            DO 100 I = 1, LB
               B( I ) = 1.0E0 + REAL( I )/( LB + 1 )
  100       CONTINUE
            DO 110 I = 1, LC
               C( I ) = 1.0E0 + REAL( I )/( LC + 1 )
  110       CONTINUE
*
*           Print header.
*
  ...
  120          CONTINUE
            END IF
            IF( NAMES( ISUB ).EQ.'SGEMM ' )THEN
               DO 160 ITA = 1, NTRANS
                  DO 150 ITB = 1, NTRANS
                     DO 140 IKVAL = 1, NKVALS
                        DO 130 ILDA = 1, NLDA
                           LDA = LDAVAL( ILDA )
                           CALL STIM01( TRANS( ITA ), TRANS( ITB ),
  ...
  150             CONTINUE
  160          CONTINUE
            ELSE IF( NAMES( ISUB ).EQ.'SSYMM ' )THEN
               DO 190 ISIDE = 1, NSIDES
                  DO 180 IUPLO = 1, NUPLOS
                     DO 170 ILDA = 1, NLDA
                        LDA = LDAVAL( ILDA )
                        CALL STIM02( SIDES( ISIDE ), UPLOS( IUPLO ),
  ...
  180             CONTINUE
  190          CONTINUE
            ELSE IF( NAMES( ISUB ).EQ.'SSYRK ' )THEN
               DO 220 IUPLO = 1, NUPLOS
                  DO 210 ITB = 1, NTRANS
                     DO 200 ILDA = 1, NLDA
                        LDA = LDAVAL( ILDA )
                        CALL STIM03( UPLOS( IUPLO ), TRANS( ITB ), NMAX,
  ...
  210             CONTINUE
  220          CONTINUE
            ELSE IF( NAMES( ISUB ).EQ.'SSYR2K' )THEN
               DO 250 IUPLO = 1, NUPLOS
                  DO 240 ITB = 1, NTRANS
                     DO 230 ILDA = 1, NLDA
                        LDA = LDAVAL( ILDA )
                        CALL STIM04( UPLOS( IUPLO ), TRANS( ITB ), NMAX,
     $                               NVALS, NNVALS, KVALS, NKVALS, A,
  ...
  240             CONTINUE
  250          CONTINUE
            ELSE IF( NAMES( ISUB ).EQ.'STRMM ' )THEN
               DO 290 ISIDE = 1, NSIDES
                  DO 280 IUPLO = 1, NUPLOS
                     DO 270 ITA = 1, NTRANS
                        DO 260 ILDA = 1, NLDA
                           LDA = LDAVAL( ILDA )
                           CALL STIM05( SIDES( ISIDE ), UPLOS( IUPLO ),
  ...
  280             CONTINUE
  290          CONTINUE
            ELSE IF( NAMES( ISUB ).EQ.'STRSM' )THEN
               DO 330 ISIDE = 1, NSIDES
                  DO 320 IUPLO = 1, NUPLOS
                     DO 310 ITA = 1, NTRANS
                        DO 300 ILDA = 1, NLDA
                           LDA = LDAVAL( ILDA )
                           CALL STIM06( SIDES( ISIDE ), UPLOS( IUPLO ),
     $                                  TRANS( ITA ), NMAX, MVALS,
     $                                  NMVALS, NVALS, NNVALS, A, LDA,
     $                                  B, LDA, LDRES,
     $                                  RESLTS( 1, 1, ILDA ) )
...
*
 ...
 9996 FORMAT( /1X, 'SGEMM with TRANSA = ''', A, ''', TRANSB = ''', A,
     $      '''' )
 9995 FORMAT( /1X, 'K = ', I4, / )
 9994 FORMAT( /1X, 'SSYMM with SIDE = ''', A, ''', UPLO = ''', A, '''',
     $      / )
 9993 FORMAT( /1X, 'SSYRK with UPLO = ''', A, ''', TRANS = ''', A, '''',
     $      / )
 9992 FORMAT( /1X, 'SSYR2K with UPLO = ''', A, ''', TRANS = ''', A,
     $      '''', / )
 9991 FORMAT( /1X, 'STRMM with SIDE = ''', A, ''', UPLO = ''', A, ''',',
     $      ' TRANS = ''', A, '''', / )
 9990 FORMAT( /1X, 'STRSM with SIDE = ''', A, ''', UPLO = ''', A, ''',',
     $      ' TRANS = ''', A, '''', / )
 9989 FORMAT( ///// )
      END
*
...
*
 ...
 9996 FORMAT( 5X, 12F9.2 )
      END
*
      SUBROUTINE STIM01( TRANSA, TRANSB, NMAX, MVALS, NMVALS, NVALS,
...
*     .. Array Arguments ..
      REAL               A( LDA, NMAX ), B( LDB, NMAX ), C( LDC, NMAX ),
     $                   RESLTS( LDRES, NNVALS )
...
*           Re-initialise C each time to avoid overflow.
   ...
   40 CONTINUE
      ...
      END
*
      SUBROUTINE STIM02( SIDE, UPLO, NMAX, MVALS, NMVALS, NVALS, NNVALS,
...
*           Re-initialise C each time to avoid overflow.
   ...
   20       CONTINUE
            S0 = SECOND( )
            S1 = SECOND( )
            CALL SSYMM( SIDE, UPLO, M, N, ALPHA, A, LDA, B, LDB, BETA,
     $                  C, LDC )
            S2 = SECOND( )
            TIME = ( S2 - S1 ) - ( S1 - S0 )
            IF( SIDE.EQ.'L' )THEN
   ...
   40 CONTINUE
      ...
      END
*
      SUBROUTINE STIM03( UPLO, TRANS, NMAX, NVALS, NNVALS, KVALS,
...
*           Re-initialise C each time to avoid overflow.
   ...
   40 CONTINUE
      ...
      END
*
      SUBROUTINE STIM04( UPLO, TRANS, NMAX, NVALS, NNVALS, KVALS,
...
*           Re-initialise C each time to avoid overflow.
   ...
   20       CONTINUE
            S0 = SECOND( )
            S1 = SECOND( )
            CALL SSYR2K( UPLO, TRANS, N, K, ALPHA, A, LDA, B, LDB, BETA,
     $                   C, LDC )
            S2 = SECOND( )
            TIME = ( S2 - S1 ) - ( S1 - S0 )
            NOP = 2*N*N*K
   ...
   40 CONTINUE
      ...
      END
*
      SUBROUTINE STIM05( SIDE, UPLO, TRANSA, NMAX, MVALS, NMVALS, NVALS,
...
*           Re-initialise B each time to avoid overflow.
            DO 20 J = 1, N
               DO 10 I = 1, M
                  B( I, J ) = 1.0E0 + REAL( I + ( J - 1 )*N )/
     $                        ( M*N + 1 )
   10          CONTINUE
   20       CONTINUE
            S0 = SECOND( )
            S1 = SECOND( )
            CALL STRMM( SIDE, UPLO, TRANSA, 'N', M, N, ALPHA, A, LDA, B,
     $                  LDB )
            S2 = SECOND( )
            TIME = ( S2 - S1 ) - ( S1 - S0 )
            IF( SIDE.EQ.'L' )THEN
   ...
   40 CONTINUE
      ...
      END
*
      SUBROUTINE STIM06( SIDE, UPLO, TRANSA, NMAX, MVALS, NMVALS, NVALS,
     $                   NNVALS, A, LDA, B, LDB, LDRES, RESLTS )
*     Times STRSM.
*     .. Parameters ..
      REAL               ALPHA
...
*           Re-initialise B each time to avoid overflow.
   ...
   20       CONTINUE
            S0 = SECOND( )
            S1 = SECOND( )
            CALL STRSM( SIDE, UPLO, TRANSA, 'N', M, N, ALPHA, A, LDA, B,
     $                  LDB )
            S2 = SECOND( )
            TIME = ( S2 - S1 ) - ( S1 - S0 )
            IF( SIDE.EQ.'L' )THEN
